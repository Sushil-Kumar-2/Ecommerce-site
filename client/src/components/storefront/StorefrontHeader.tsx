      ...
      <div className="border-b bg-brand-primary md:bg-background">
        <div className="mx-auto flex max-w-7xl items-center gap-2 px-4 py-2 md:gap-4 md:py-3">
          ...
          <div className="ml-auto flex items-center gap-0.5 sm:gap-1">
            {isAuthenticated && user ? (
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="hidden h-auto flex-col gap-0.5 px-2 py-1 text-white hover:bg-white/10 md:flex md:text-foreground md:hover:bg-muted"
                  >
                    <User className="size-5" />
                    <span className="max-w-20 truncate text-[10px] font-normal">
                      {user.email.split('@')[0]}
                    </span>
                  </Button>
                </DropdownMenuTrigger>
              ...
              >
                <Link to={ROUTES.login}>
                  <User className="size-5" />
                  <span className="text-[10px] font-normal">Login</span>
                </Link>
              </Button>
            )}
            ...
            <Button
              variant="ghost"
              className="hidden h-auto flex-col gap-0.5 px-2 py-1 text-white hover:bg-white/10 sm:flex md:text-foreground md:hover:bg-muted"
              onClick={() => handleGuestProtected(ROUTES.orders)}
            >
              <Package className="size-5" />
              <span className="text-[10px] font-normal">Returns &amp; Orders</span>
            </Button>
            ...
            >
              ...
              {isAuthenticated && wishlistCount > 0 ? (
                <Badge className="absolute -top-1 -right-1 size-5 min-w-5 bg-brand-accent px-1 text-[10px] text-foreground">
